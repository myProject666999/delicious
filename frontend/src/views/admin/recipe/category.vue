<template>
  <div class="admin-recipe-category">
...
<script>
import { getRecipeCategoryPage, saveRecipeCategory, deleteRecipeCategory } from '@/api/recipe'
  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        ...
        const res = await getRecipeCategoryPage(params)
    ...
    handleStatus(row) {
      ...
      this.$confirm(`确定要${action}该分类吗？`, '提示', {
        ...
        try {
          await saveRecipeCategory({ id: row.id, status: newStatus })
    ...
    async handleSubmit() {
      ...
      try {
        await saveRecipeCategory(this.form)
