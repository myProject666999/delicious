      ...
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="image" label="图片" width="200">
          <template slot-scope="scope">
            <el-image :src="scope.row.image" style="width: 160px; height: 80px;" fit="cover"></el-image>
          </template>
        </el-table-column>
        <el-table-column prop="title" label="标题" min-width="150" show-overflow-tooltip></el-table-column>
        <el-table-column prop="linkUrl" label="跳转链接" min-width="150" show-overflow-tooltip></el-table-column>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="600px">
      <el-form :model="form" :rules="rules" ref="form" label-width="80px">
        ...
        <el-form-item label="图片" prop="image">
          ...
          <div class="tips">建议图片尺寸: 1920 x 400像素</div>
        </el-form-item>
        <el-form-item label="跳转链接">
          <el-input v-model="form.linkUrl" placeholder="请输入跳转链接（可选）"></el-input>
        </el-form-item>
...
<script>
import { getBannerPage, saveBanner, deleteBanner } from '@/api/banner'
  ...
  methods: {
    ...
    handleStatus(row) {
      ...
      this.$confirm(`确定要${action}该轮播图吗？`, '提示', {
        ...
        try {
          await saveBanner({ id: row.id, status: newStatus })
